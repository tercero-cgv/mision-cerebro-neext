...
async function startEscapeRoom(day) {
  const keySnap = await db.ref("config/anthropicKey").once("value");
  const anthropicKey = keySnap.val();
  if (!anthropicKey) { renderEscapeRoom(getFallbackEscape(day.topic)); return; }
  $('game-area').innerHTML = `
